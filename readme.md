...
- 4.记录关键节点列表（按照主观感觉写）
- 5.当前app没有的意图，除了意图，直接全部置[]
- 6.注意!!! 确保len(action_desp_list) == len(bounding_box_list) == len(image_path_list) -1，方便后续解析
...
- windows/mac 可以通过adb/hdc命令行对 安卓/鸿蒙手机 当前页面进行截图，直接保存到电脑
  ```python
